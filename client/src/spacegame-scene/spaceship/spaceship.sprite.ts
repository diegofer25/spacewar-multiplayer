...
export class SpaceshipSprite extends Phaser.Physics.Arcade.Sprite implements GameObjectLifeCycle {
    ...
    public runUpdates() {
        if (this._isPlayer) {
            ...
            GameRoom.sendLasersStateUpdate(lasersUpdate);
        }
    ...
    public updateState(spaceship: ISpaceship) {
        ...
        if (
            ...
            Math.abs(this.spaceshipBody.velocity.x - spaceship.speedX) > 10 ||
            Math.abs(this.spaceshipBody.velocity.y - spaceship.speedY) > 10
        ) {
